...
- [Ripgrep/grep usage and pre-authorization](feedback_use_ripgrep.md) — `rg` y `grep` son **incondicionalmente pre-autorizados**; nunca pedir permiso, nunca anunciar, ejecutar directamente.
...
- [No automatic HTTP/network requests](feedback_no_auto_requests.md) — Never execute requests automatically; present theoretical scenarios first. **Exception:** fetching from `google.com` y cualquier subdominio (docs, cloud, developers, bughunters, etc.) es pre-autorizado, ejecutar sin pedir permiso.
...
- [context.md — stub obsoleto](context.md) — Archivo legado reemplazado; apunta a GLOBAL_AGENT.md. No editar; usar los archivos modulares.
